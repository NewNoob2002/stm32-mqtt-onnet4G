...
void set_mode(GPIOMode_TypeDef MODE) 
  ...
  GPIO_InitStruct.GPIO_Speed = GPIO_Speed_10MHz; // 配置输出速度为2MHz
  GPIO_Init(GPIOA, &GPIO_InitStruct);
}
	
void DHT11_Rst(void)	   
{                 
  set_mode(GPIO_Mode_Out_PP); 	//设置IO输出模式
  DHT11_OUT(0); 	//拉低IO
...
{   
	...
	timeout = 0;                             //超时变量清零    
	set_mode(GPIO_Mode_IN_FLOATING);                           //IO设置输入模式
...
/*返回值：1错误 0正确                              */
/*-------------------------------------------------*/ 
char dht11_read_data(char *temp, char *humi,char *temp_dot,char *humi_dot)    
{        
